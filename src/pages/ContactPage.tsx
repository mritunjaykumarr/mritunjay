...
export default function ContactPage() {
  ...
  return (
    <div className="page-wrapper contact-page" style={{ paddingTop: '6rem', paddingBottom: '5rem', background: 'var(--bg)', color: 'var(--text)', minHeight: '100vh' }}>
      {/* Page Header */}
      ...
      <section className="section" style={{ padding: '2rem 0 4rem' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
            ...
            <div>
              <div style={{ padding: '1.5rem', marginBottom: '1.5rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e', display: 'inline-block' }} />
                  <span style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text)' }}>Available for Select Projects</span>
                </div>
                <p style={{ fontSize: '0.86rem', color: 'var(--text-muted)', marginTop: '0.5rem', lineHeight: 1.6, margin: '0.5rem 0 0' }}>
                  Currently accepting new product engineering contracts, full-stack roles, and AI workflow consultancies.
                </p>
              </div>

              <div style={{ padding: '1.75rem', marginBottom: '1.5rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1.25rem', color: 'var(--text)' }}>Direct Channels</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  {[
                    ...
                    <a
                      ...
                      rel="noreferrer"
                      style={{
                        display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text)',
                        textDecoration: 'none', padding: '0.75rem', borderRadius: '8px',
                        background: 'var(--surface-2)', border: '1px solid var(--border)'
                      }}
                    >
                      <div style={{ width: 36, height: 36, borderRadius: '6px', background: 'var(--surface-3)', display: 'grid', placeItems: 'center', color: 'var(--text)', flexShrink: 0 }}>
                        <item.Icon size={16} />
                      </div>
                      <div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{item.label}</div>
                        <div style={{ fontWeight: 500, fontSize: '0.88rem', color: 'var(--text)' }}>{item.value}</div>
                      </div>
                    </a>
                  ))}
                </div>
              ...
              <div style={{ padding: '1.5rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text)', fontSize: '0.88rem' }}>
                ...
                <div style={{ marginTop: '0.75rem', fontSize: '1.2rem', fontWeight: 600, color: 'var(--text)' }}>
            ...
            {/* Right Form */}
            <div>
              <div style={{ padding: '2.25rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px' }}>
                <div className="badge-playful" style={{ marginBottom: '0.75rem' }}>
                  <Sparkles size={12} />
                  <span>Send a message</span>
                </div>
                <h3 style={{ fontSize: '1.35rem', fontWeight: 600, marginBottom: '0.5rem', color: 'var(--text)' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'grid', gap: '4px' }}>
                    <label htmlFor="c-page-email" style={{ fontSize: '0.76rem', color: 'var(--text-muted)' }}>Email Address</label>
                    <input type="email" name="email" placeholder="john@company.com" required id="c-page-email" />
                  </div>

                  <div style={{ display: 'grid', gap: '4px' }}>
                    <label htmlFor="c-page-subject" style={{ fontSize: '0.76rem', color: 'var(--text-muted)' }}>Subject / Project Scope</label>
                    <input type="text" name="subject" placeholder="New product build, AI workflow, full stack role..." id="c-page-subject" />
                  </div>

                  <div style={{ display: 'grid', gap: '4px' }}>
                    <label htmlFor="c-page-message" style={{ fontSize: '0.76rem', color: 'var(--text-muted)' }}>Your Message</label>
              ...
              <div style={{ marginTop: '1.5rem', padding: '1.5rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
                <h4 style={{ fontSize: '0.92rem', fontWeight: 600, color: 'var(--text)', marginBottom: '1rem' }}>Frequently Asked Questions</h4>
                    ...
                    <div key={i} style={{ borderBottom: '1px solid var(--border)', paddingBottom: '0.75rem' }}>
                      <button
                        ...
                        style={{
                          ...
                          textAlign: 'left', color: 'var(--text)', fontSize: '0.86rem', fontWeight: 500, cursor: 'pointer', padding: 0
                        }}
                      >
                        <span>{faq.q}</span>
                      ...
                      {openFaq === i && (
                        <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '0.5rem', lineHeight: 1.6, margin: '0.5rem 0 0' }}>
                          {faq.a}
                        </p>
                      )}
                    </div>
